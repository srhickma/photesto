...
window.onload = function(){
  if($(window).width() < 825){
    ...
    document.getElementById("resetform").style.padding = "4px";
  }
  $.ajax({
      type:"GET",
      url:'/SGetUser.php',
...
$(document).ready(function(){
    ...
    $('#resetbtn').click(function(event){
        ...
        var usr = vars[0].split("=")[1];
        var code = vars[1].split("=")[1];
        var pas = document.resetform.pas.value;
        var cpas = document.resetform.cpas.value;
        event.preventDefault();
        $.ajax({
            type:"POST",
            url:'/SResetPass.php',
            data:{usr:usr, code:code, pas:pas, cpas:cpas},
            success:function(resp){
                removeLoader();
                if(resp === "S")window.location="http://photesto.space/login.html";
                else{
                    document.getElementById("resetmsg").innerHTML = resp;
